...
❗️ ➜ EXAMPLE: /customvoice number spoof service name
...
🔑 To activate the bot, type /purchase Or contact @CKRACKING_MOROCCO.
  ...
  const args = ctx.message.text.split(" ").slice(1);
  if (args.length < 4) {
    await ctx.reply("❗️ Usage: /call victim_number spoof_number service name");
    return;
  }

  const [victimNumber, spoofNumber, serviceRaw, nameRaw] = args;
  const service = serviceRaw.charAt(0).toUpperCase() + serviceRaw.slice(1).toLowerCase();
  const maskedName = nameRaw.charAt(0).toUpperCase() + "*".repeat(Math.max(3, nameRaw.length - 1));
  const otpCode = generateOtp();

  await ctx.reply(`📞 Call has been Started to ${victimNumber} with ${nameRaw}`, {
    reply_markup: {
      inline_keyboard: [
        [{ text: "Hangup ✖️", callback_data: "hangup_call" }]
      ]
    }
  });

  await ctx.reply("📞 Call In Progress...\n• Call Has Been Answered By Victim 📂");
  await ctx.reply("🤖 Silent Human detection");
  await ctx.reply("⏳ Victim Waiting For The OTP...\n📲 Send The OTP Now.");
  await ctx.reply(`✅ Verification OTP Code: ${otpCode}`);

  await ctx.replyWithAudio({ source: "./LAZARUS_O-T-P.mp3" });

  await ctx.reply("Choose an option:", {
    reply_markup: {
      inline_keyboard: [
        [{ text: "Accept ✅", callback_data: "accept_call" }, { text: "Deny ❌", callback_data: "deny_call" }]
      ]
    }
  });

  await ctx.reply("📞 Call has ended.");
  ...
  try {
    ...
    console.error("❌ Failed to send message to channel:", e.message);
  }
});

bot.callbackQuery("hangup_call", async (ctx) => {
  await ctx.answerCallbackQuery();
  await ctx.reply("📴 Call has been canceled.");
});

bot.callbackQuery("accept_call", async (ctx) => {
  await ctx.answerCallbackQuery();
  await ctx.reply("✅ You accepted the call.");
});

bot.callbackQuery("deny_call", async (ctx) => {
  await ctx.answerCallbackQuery();
  await ctx.reply("❌ You denied the call.");
});
...
async function sendRandomMessages() {
  while (true) {
    ...
    const maskedUsername = name[0] + name.slice(1).replace(/./g, "*");
